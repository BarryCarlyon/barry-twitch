...
describe("Twitch Utilities", () => {
    ...
    */
    ...
    it("Create Pin Duration Too Short", async () => {
        ...
        await expect(tw.createPinnedChatMessage("123123", "321321", "abc-abc-abc", 5)).to.be.rejectedWith(
            Error,
            /^Duration Seconds is too short, less than 30$/,
        );
        //expect(chatResponse.status).to.be.equal(200);
        //expect(apiNock).to.have.been.requested;
        expect(validateNock).to.have.been.requested;
    ...
    it("Create Pin Duration Too Long", async () => {
        ...
        await expect(tw.createPinnedChatMessage("123123", "321321", "abc-abc-abc", 1805)).to.be.rejectedWith(
            Error,
            /^Duration Seconds is too long, greater than 1800$/,
        );
        //expect(chatResponse.status).to.be.equal(200);
        //expect(apiNock).to.have.been.requested;
        expect(validateNock).to.have.been.requested;
    ...
    it("Create Pin With Duration", async () => {
        ...
        let tw = new Twitch(VALID_CLIENT_ID);
        await tw.setToken(VALID_TOKEN);
        let chatResponse = await tw.createPinnedChatMessage("123123", "321321", "abc-abc-abc", 1000);
